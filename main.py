...
questions = ["0. Сколько зубов во рту у человека?",
...
answers = ["32", "Стартап", "Инвестор", "Инвестиция", "MVP", "Бизнес-план", "Конкуренты", "Прибыль"]
score = 0
tooth = False
print(questions[0])
...
if tooth:
    tooth = "Ты достаточно знаешь о зубах."
else:
    tooth = ""

if score > 5:
    print(f"Это отличный результат! Ты много знаешь о стартапах. {tooth}")
elif score > 3:
    print(f"Неплохой результат! Как здорово, что тебе предстоит узнать ещё так много о стартапах. {tooth}")
else:
    print(f"Неплохой результат! Как здорово, что тебе предстоит узнать ещё так много о стартапах. {tooth}")